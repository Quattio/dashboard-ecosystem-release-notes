...
4. **Release timeline (live).** Every page includes a `<section class="timeline-wrap" id="release-timeline" hidden>` just before the tabs, filled at load by a small `fetch('/api/calendar')`. That endpoint is a Cloudflare Pages Function (`functions/api/calendar.ts`) which reads the shared Quatt release calendar's **secret iCal feed** server-side (behind the existing quatt.io auth) and returns the events as JSON. Set a JS `const RELEASE_DATE = 'YYYY-MM-DD'` in that block to this wave's date, so the matching calendar event is marked "this release". The section **stays hidden** if the feed is unset or unreachable — it never breaks a page. **One-time infra, not per release:** the function and the `RELEASE_TIMELINE_ICS_URL` Cloudflare Pages secret must exist (Google Calendar → Settings → the calendar → "Secret address in iCal format"). **Never commit the feed URL** — it lives only in the secret.
