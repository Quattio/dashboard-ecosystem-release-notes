...
1. **`releases/YYYY-MM-DD.html`** — one self-contained page per release wave (date = CiC tag creation date). **Filename and branch use `YYYY-MM-DD`** to match the repo's existing `releases/` pages and the `releases.js` page paths; the human-facing H1 title still uses `DD.MM.YYYY` (dots) per Step 6. **Always build the page by copying the canonical template `releases/_template.html`** (relative paths are one level deep: `../styles.css`, `../index.html`). Structure:
   > **Build from the template, never clone a previous dated release page.** Older `releases/*.html` pages may be in a superseded layout (e.g. the pre-epic-grouped numbered-theme format), and cloning one silently carries the old format forward. This applies to **PROMOTE-in-place** too: when updating a wave that already has an entry, port the existing entry's *content* into a fresh copy of `releases/_template.html` — do not edit the old page's markup in place — then rename/replace as needed so there is one page per wave.
...
4. **Release timeline (live, per-version).** Every page includes a `<section class="timeline-wrap" id="release-timeline" hidden>` just before the tabs, filled at load by `fetch('/api/calendar')`. That endpoint (`functions/api/calendar.ts`) reads the shared **"Embedded Software Release cycle"** calendar's **secret iCal feed** server-side (behind the quatt.io auth) and returns concrete events as JSON — un-expanded recurring masters (with an `RRULE`) are skipped; one-off events and per-occurrence overrides are kept. Set two consts in the timeline block: `RELEASE_DATE = 'YYYY-MM-DD'` (headline date) and `RELEASE_VERSION = 'X.Y.Z'` (base CiC version, **no** pre-release suffix). The renderer shows the milestones **whose title contains that version** — this release's own cycle (Package alpha → Test → Beta → Fix → Roll out → Full roll out) — each with its **date range** (all-day iCal end dates are exclusive, so subtract a day for the inclusive last day). If no milestone carries the version, it falls back to a ±6-week window around `RELEASE_DATE`; if still empty (or on any feed/endpoint error) the section **stays hidden**, so it never breaks a page. **Dependency:** the per-version timeline is only complete when each cycle's milestones are tagged `| CiC X.Y.Z` on the calendar. **One-time infra, not per release:** the function + the `RELEASE_TIMELINE_ICS_URL` Cloudflare Pages secret must exist (Google Calendar → Settings → the calendar → "Secret address in iCal format"). **Never commit the feed URL** — it lives only in the secret.
