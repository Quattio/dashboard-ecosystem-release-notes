...
### 2026-08-14 — Tier 1 never exposes release-engineering internals (alpha/beta, test build, fleet-rollout status); state the customer outcome plainly
- **Category**: tier-placement
- **Trigger**: PR #32 — "this message is not something we should have in the customer facing front \"The CiC 4.12.0 firmware in this update is an early (alpha) test build and is not yet on the customer fleet, so everyday heating and comfort behaviour is unchanged for customers in this release.\" Keep it more concise, like \"Everyday heating and comfort behaviour is unchanged in this release\"" (alex-aristotelous)
- **Root cause**: On an alpha wave whose CiC firmware is not customer-live, generation justified the "nothing changed for you" reassurance in the Customer tab by explaining the release mechanics — that the firmware is "an early (alpha) test build", "not yet on the customer fleet". That rationale is internal release-engineering context (phase, rollout status) that belongs in Tier 2/Tier 3, not on the homeowner-facing surface; the customer only needs the outcome, stated briefly.
- **Rule**: Tier 1 (EN + NL, and DE/SK when present) must never surface release-engineering internals — the words "alpha"/"beta"/"test build"/"pre-release", fleet/rollout status ("not yet on the customer fleet", "goes to alpha testers"), or any explanation of *why* a change is or isn't live. State only the customer-perceivable outcome, concisely (e.g. "Everyday heating and comfort behaviour is unchanged in this release." / "Het dagelijkse verwarmings- en comfortgedrag is ongewijzigd in deze release."). All phase/rollout framing lives in the staging-only review panel and in Tier 2/Tier 3. This complements the 2026-07-06 "Tier 2 stays at operational altitude" rule (keep engineering depth out of Tier 2) by keeping release-mechanics depth out of Tier 1 as well — and note it does not change phase *classification* (the tag suffix still governs that, per 2026-07-27); it only governs what customer copy is allowed to say.

### 2026-08-14 — A ticket key in the diff ≠ all its work shipped; describe each item by the commit that landed in THIS wave and verify the component version bumped
- **Category**: scope
- **Trigger**: PR #32 — "The \"Word/block flash programming in the NVMEM driver\" item was not in this release, you can remove it." (ramacassis)
- **Root cause**: QPD-14524 spans two commits — a `quatt-heatcharger-firmware` word/block flash-programming (NVMEM) feature merged 2026-06-29, and a `quatt-embedded-cpp-apps` "poll HeatCharger install state promptly" commit that shipped this wave. Generation detected the QPD key in the wave's CiC diff, then wrote the item from the ticket's *firmware* commit. That firmware feature did not ship this wave: HeatCharger FW is unchanged at 1.3.0 (the page's own engineering footer and epic-desc both said so), yet the item was presented as a shipped HeatCharger improvement.
- **Rule**: A QPD key appearing in a wave's commit diff does NOT mean all of that ticket's work shipped. When a ticket spans multiple commits/repos, describe (and decide inclusion of) each release-notes item strictly by the commit(s) that landed in THIS wave's bumped components — never by the ticket title or a sibling commit in a different repo/wave. Before listing any firmware/hardware item, confirm that component's version actually bumped this wave (HeatCharger FW, Thread FW, ODU FW, Chill CB FW, Controller, …); if the version is unchanged, that ticket's firmware-side work did not ship — omit it from all tiers even when the shared QPD key also has a shipped CiC/App/Cloud commit. The tell is a mismatch between an item's described change and the "unchanged" version stated in the same page's component-bump footer. This sharpens the 2026-07-06 "Hold cross-product features until the counterpart ships" rule into a concrete check: match every item's described change to a component whose version bumped, not merely to a ticket key present in the diff.
