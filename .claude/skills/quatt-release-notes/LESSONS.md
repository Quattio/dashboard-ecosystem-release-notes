...
### 2026-07-27 — Pair a just-past-window release when it is what enables the wave's shipped-but-disabled features
- **Category**: versioning
- **Trigger**: PR #26 — "pair this release with App v1.60.0 and regenerate release notes" (alex-aristotelous)
- **Root cause**: Auto-pairing treated the +7d tail as a hard boundary and excluded App v1.60.0 (shipped 2026-07-24, one day past the 2026-07-23 window end), deferring it to the "next" wave — even though v1.60.0 is precisely the release that removes the app-side feature gates (`insights_v2` via QPD-14883, `all_e_boost_mode` via QPD-14724) and ships the day/night sound-schedule UI (QPD-15101), i.e. the release that turns three of *this* wave's shipped-but-disabled features live. The wave's feature code and the release that enables it were split across two waves' notes, leaving those features wrongly marked "do not communicate".
- **Rule**: The +7d pairing tail is a heuristic, not a hard cutoff. Before finalising, for **every** feature you would mark shipped-but-disabled, check whether the *next* paired-product release — even one landing just past the window — is the one that enables it (removes the gating flag, flips the config default, or ships the counterpart UI/firmware). If so, pair that release with THIS wave, move the feature from shipped-but-disabled to live across all tiers (Tier 1 EN+NL, Tier 2 "now live", Tier 3 rows), and drop the "do not communicate" callout. Do not split a feature's code and its enabling release across two waves. (Complements the 2026-07-06 enablement-sweep lesson: that one covers enablement *tickets* with no commit in the diff; this one covers a whole paired *release* that falls just outside the window.)
